...
function freqim =  freqest(im, orientim, windsze, minWaveLength, maxWaveLength)
    ...
    %figure(11),subplot(2,2,1),imshow(rotim),title("rotated");
    ...
    %subplot(2,2,3),imshow(rotim),title("croped");
    ...
    maxpts = (dilation == proj) & (proj > mean(proj)) & (proj - mean(proj) > cropsze/4);
    maxind = find(maxpts(1:end));
