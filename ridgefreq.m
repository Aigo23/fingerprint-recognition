...
function [freq, medianfreq] = ridgefreq(im, mask, orient, blksze, windsze, ...
    ...
    %/--add
    slide = blksze / 2;
    for r = 1+blksze/4:slide:rows-blksze+blksze/4
        for c = 1+blksze/4:slide:cols-blksze+blksze/4
          blkim = im(r-blksze/4:r+blksze*3/4-1, c-blksze/4:c+blksze*3/4-1);    
          blkor = orient(r-blksze/4:r+blksze*3/4-1, c-blksze/4:c+blksze*3/4-1);       
          
          freq_block = freqest(blkim, blkor, windsze, minWaveLength, maxWaveLength);
          av = mean(freq_block(:));
          freq(r:r+slide-1,c:c+slide-1) =  ...
              av*ones(slide);
        end
    end
