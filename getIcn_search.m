...
function [] = getIcn_search(img, mask)

% img = imread("data1\search\img\49.png");
% mask = (imread("data1\search\mask\49.png")) > 0;
...
blksze = 16; thresh = 0.2;
    [normim, mask1] = ridgesegment(img, blksze, thresh);
...
filename = strcat('IcnSearch\Icn','.txt');
...
minu = LoadNeuFeature('IcnSearch\Icn.txt');
minu(:,3) = -minu(:,3);
DrawMinu(fig,minu);
end
